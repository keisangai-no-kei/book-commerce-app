...
export default async function ProfilePage() {
  ...
  if (user) {
    ...
    purchasedBooksDetail = await Promise.all(
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="flex items-center gap-6">
        {purchasedBooksDetail.map((purchasedBookDetail: BookType) => (
          <PurchasedBookDetail
            key={purchasedBookDetail.id}
            purchasedBookDetail={purchasedBookDetail}
          />
        ))}
      </div>
    </div>
